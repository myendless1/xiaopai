...
2. 动态表情通过独立任务按帧切换。
3. 触摸屏幕时通过 `xiaopai_state_get` 和 `expression_state_get` 读取两套状态，决定唤醒、忽略或显示临时 `shy`。
4. 空闲超过阈值后显示 `sleep_dark`。

## 关键实现
- `stack-chan/main/expression_state.cpp`: 业务层统一通过此入口读写表情状态。
...
- 普通表情修改使用 `expression_state_set(name)`，临时表情使用 `expression_state_set_temporary`。
