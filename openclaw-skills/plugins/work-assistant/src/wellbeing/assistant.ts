import type {
  InputEvent,
  SedentaryDetectedPayload,
  StructuredResponse,
  ToolAction,
  WellbeingCompanionRequestedPayload
} from "../contracts.js";
import type {
  LarkCalendarAdapter,
  LarkCalendarListRequest,
  NormalizedAgendaEvent
} from "../lark/adapters.js";

export type WellbeingDecision =
  | "allowed"
  | "low_confidence"
...
type ValidatedSedentaryPayload = SedentaryDetectedPayload & {
  duration_minutes: number;
  confidence: number;
};

type NearbyEventSummary = {
  event_id: string;
  title: string;
  start: string;
  end: string;
...
type CalendarContext =
  | {
      action: ToolAction;
      currentEvent?: NormalizedAgendaEvent;
      upcomingEvent?: NormalizedAgendaEvent;
      degraded: false;
    }
  | {
      action: ToolAction;
...
export class WellbeingCompanionAssistant {
  ...
  private readonly minimumSedentaryDurationMinutes: number;
  private readonly minimumConfidence: number;
  private readonly cooldownMinutes: number;
  private readonly upcomingReminderHorizonMinutes: number;
  ...
  async handle(event: InputEvent): Promise<StructuredResponse> {
    if (event.type === "wellbeing_companion_requested") return this.handleCompanionRequest(event);
    return this.handleSedentaryDetected(event);
  }

  private async handleSedentaryDetected(event: InputEvent): Promise<StructuredResponse> {
    ...
    if (upcomingEvent) contextPatch.wellbeing_nearby_event = summarizeEvent(upcomingEvent);
    if (calendarContext.degraded) contextPatch.wellbeing_calendar_degraded = true;
  ...
  private handleCompanionRequest(event: InputEvent): StructuredResponse {
  ...
  private async listCalendarContext(event: InputEvent): Promise<CalendarContext> {
    ...
    const request: LarkCalendarListRequest = { start, end };
    if (this.calendarId) request.calendarId = this.calendarId;
    ...
    if (currentEvent) details.current_event_id = currentEvent.id;
    if (upcomingEvent) details.upcoming_event_id = upcomingEvent.id;
    ...
    if (currentEvent) context.currentEvent = currentEvent;
    if (upcomingEvent) context.upcomingEvent = upcomingEvent;
    return context;
  }

  private evaluateCooldown(event: InputEvent):
    | { active: false }
    | { active: true; lastNudgeAt: string; minutesSinceLastNudge: number } {
    ...
    if (!lastNudgeAt) return { active: false };

    const last = Date.parse(lastNudgeAt);
    const current = Date.parse(event.timestamp);
    if (!Number.isFinite(last) || !Number.isFinite(current)) return { active: false };
  ...
  private skippedResponse(
    event: InputEvent,
    decision: Exclude<WellbeingDecision, "allowed">,
    details: Record<string, unknown>,
    precedingActions: ToolAction[] = []
  ): StructuredResponse {
    return {
      speech: "",
      presentation: {
  ...
  private evaluateAction(decision: WellbeingDecision, details: Record<string, unknown>): ToolAction {
    return {
...
function validateSedentaryPayload(payload: InputEvent["payload"]):
  | { ok: true; value: ValidatedSedentaryPayload }
  ...
  const value: ValidatedSedentaryPayload = {
    ...payload,
  ...
  if (deviceId) value.device_id = deviceId;
  return { ok: true, value };
}

function normalizeCompanionPayload(payload: InputEvent["payload"]): WellbeingCompanionRequestedPayload {
  const normalized: WellbeingCompanionRequestedPayload = {};
  ...
  if (requestType) normalized.request_type = requestType;
  if (contentType) normalized.content_type = contentType;
  if (continueRequested === true) normalized.continue_requested = true;
  return normalized;
}

function selectCompanionContentType(payload: WellbeingCompanionRequestedPayload): "joke" | "relaxation" | "light_chat" {
  const raw = `${payload.content_type ?? ""} ${payload.request_type ?? ""}`.toLocaleLowerCase();
  if (/relax|breath|calm|stretch|放松|呼吸|伸展/.test(raw)) return "relaxation";
  if (/chat|companion|陪伴|聊/.test(raw)) return "light_chat";
  return "joke";
}

function selectCompanionSpeech(contentType: "joke" | "relaxation" | "light_chat", eventId: string): string {
  const bank =
...
function buildNudgeSpeech(event: InputEvent, upcomingEvent: NormalizedAgendaEvent | undefined): string {
...
function selectCurrentEvent(events: NormalizedAgendaEvent[], at: number): NormalizedAgendaEvent | undefined {
  return events
    .filter((event) => {
...
): NormalizedAgendaEvent | undefined {
...
function summarizeEvent(event: NormalizedAgendaEvent): NearbyEventSummary {
  const summary: NearbyEventSummary = {
    event_id: event.id,
    title: event.title,
    start: event.start,
    end: event.end
  };
  if (event.calendarId) summary.calendar_id = event.calendarId;
  return summary;
}
...
function readContextString(context: InputEvent["context"], key: string): string | undefined {
  const value = context[key];
  return typeof value === "string" && value.trim() !== "" ? value : undefined;
}

function readPayloadString(payload: InputEvent["payload"], key: string): string | undefined {
