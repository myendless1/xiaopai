...
import type { MeetingNotificationTarget } from "./contracts.js";
import {
  ...
  readSchedulerConfig,
  type SchedulerAgentDispatchRuntimeApi,
  type ProactiveCalendarSchedulerConfig
...
} from "./travel/dry-run.js";
import { QWeatherWeatherAdapter } from "./travel/qweather.js";
import { WellbeingCompanionAssistant, type WellbeingCompanionConfig } from "./wellbeing/assistant.js";

type PluginConfig = {
  dryRun?: boolean;
  larkCliPath?: string;
  larkIdentity?: "user" | "bot";
  imProvider: "openclaw" | "lark-cli";
  openclawCliPath?: string;
  openclawFeishuChannel?: string;
  openclawFeishuAccount?: string;
  defaultMeetingNotificationChatId?: string;
  timeoutMs?: number;
...
function readPluginConfig(api: { pluginConfig?: unknown }): PluginConfig {
  ...
  if (typeof raw.larkCliPath === "string") config.larkCliPath = raw.larkCliPath;
  if (typeof raw.openclawCliPath === "string") config.openclawCliPath = raw.openclawCliPath;
  if (typeof raw.openclawFeishuChannel === "string") config.openclawFeishuChannel = raw.openclawFeishuChannel;
  if (typeof raw.openclawFeishuAccount === "string") config.openclawFeishuAccount = raw.openclawFeishuAccount;
  ...
  if (travel) config.travel = travel;
  const wellbeing = readWellbeingConfig(raw.wellbeing);
  if (wellbeing) config.wellbeing = wellbeing;
  return config;
}

export function createDefaultWorkAssistant(api: { pluginConfig?: unknown }) {
  return createDefaultWorkAssistantRuntime(api).assistant;
}

export function createDefaultWorkAssistantRuntime(api: RuntimeApi) {
  ...
  const assistant = createWorkAssistantHandler({
    ...
    meetingReminderAssistant: new MeetingReminderAssistant({
      ...
      ...(config.defaultMeetingNotificationChatId
        ? { defaultNotificationTarget: { chat_id: config.defaultMeetingNotificationChatId } satisfies MeetingNotificationTarget }
        : {})
    }),
    ...
    ? new ProactiveCalendarTriggerScheduler({
        ...
        dispatch: async (event) => {
          ...
          if (agentDispatch.status === "failed") {
            api.logger?.warn?.(
              `work-assistant scheduler agent dispatch failed ${JSON.stringify({
...
export const workAssistantPlugin = definePluginEntry({
  ...
  register(api) {
    ...
    if (runtime.scheduler && runtime.config.scheduler.startIntervalLoop) {
      ...
      registerDisposal(api as DisposalApi, () => loop?.stop());
    }
  }
});
...
function readTravelConfig(value: unknown): TravelPlannerConfig | undefined {
  const raw = typeof value === "object" && value !== null && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
  const config: TravelPlannerConfig = {};
  ...
  if (originAddress) config.originAddress = originAddress;
  if (defaultRouteMode) config.defaultRouteMode = defaultRouteMode;
  if (arrivalBufferMinutes !== undefined) config.arrivalBufferMinutes = Math.floor(arrivalBufferMinutes);
  if (route) config.route = route;
  if (weather) config.weather = weather;
  return Object.keys(config).length > 0 ? config : undefined;
}

function readAmapRouteConfig(value: unknown): AmapRouteConfig | undefined {
  const raw = typeof value === "object" && value !== null && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
  if (raw.provider !== "amap") return undefined;
  const config: AmapRouteConfig = {
    provider: "amap"
  };
  ...
  if (credentialEnv) config.credentialEnv = credentialEnv;
  if (defaultCity) config.defaultCity = defaultCity;
  if (originLocation) config.originLocation = originLocation;
  if (timeoutMs !== undefined) config.timeoutMs = timeoutMs;
  return config;
}

function readQWeatherConfig(value: unknown): TravelPlannerConfig["weather"] | undefined {
  const raw = typeof value === "object" && value !== null && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
  if (raw.provider !== "qweather") return undefined;
  const config: NonNullable<TravelPlannerConfig["weather"]> = {
    provider: "qweather"
  };
  const apiHost = readNonEmptyString(raw.apiHost);
  const credentialEnv = readNonEmptyString(raw.credentialEnv);
  ...
  if (defaultUnit) config.defaultUnit = defaultUnit;
  if (defaultRange) config.defaultRange = defaultRange;
  if (forecastDays) config.forecastDays = forecastDays;
  if (timeoutMs !== undefined) config.timeoutMs = timeoutMs;
  return config;
}

function readWellbeingConfig(value: unknown): WellbeingCompanionConfig | undefined {
  const raw = typeof value === "object" && value !== null && !Array.isArray(value) ? (value as Record<string, unknown>) : {};
  const config: WellbeingCompanionConfig = {};
  ...
  if (minimumConfidence !== undefined) config.minimumConfidence = minimumConfidence;
...
function readRouteMode(value: unknown): RouteMode | undefined {
  return value === "driving" || value === "transit" || value === "walking" ? value : undefined;
}

function readQWeatherAuthMode(value: unknown): QWeatherAuthMode | undefined {
  return value === "apiKeyHeader" || value === "apiKeyQuery" || value === "jwtBearer" ? value : undefined;
}

function readQWeatherUnit(value: unknown): QWeatherUnit | undefined {
  return value === "metric" || value === "imperial" ? value : undefined;
}

function readQWeatherForecastDays(value: unknown): QWeatherForecastDays | undefined {
  return value === 3 || value === 7 || value === 10 || value === 15 || value === 30 ? value : undefined;
}

function startSchedulerLoop(
  scheduler: ProactiveCalendarTriggerScheduler,
...
function registerDisposal(api: DisposalApi, callback: () => void): void {
  if (typeof api.onDispose === "function") api.onDispose(callback);
}
