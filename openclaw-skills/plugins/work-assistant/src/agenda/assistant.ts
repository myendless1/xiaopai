import type { InputEvent, StructuredResponse, ToolAction } from "../contracts.js";
...
export class AgendaBriefingAssistant {
  ...
  async handle(event: InputEvent): Promise<StructuredResponse> {
    let todayWindow: BriefingWindow;
    let recapWindow: BriefingWindow;
    try {
      ...
      return followUpResponse(
        "请提供有效的事件时间和时区后再生成日程简报。",
        "invalid_briefing_context",
        "日程简报需要有效的时间和时区。"
      );
    }
    ...
    const actions: ToolAction[] = [todayResult.action, recapResult.action];
  ...
  private async listWindow(
    windowType: "today" | "recap",
    window: BriefingWindow
  ): Promise<{ action: ToolAction; events: NormalizedAgendaEvent[] } | { action: ToolAction }> {
    const request: LarkCalendarListRequest = {
      start: window.start,
      end: window.end
    };
...
export function calculateTodayWindow(timestamp: string, timezone: string): BriefingWindow {
...
export function calculatePreviousWorkWeekWindow(timestamp: string, timezone: string): BriefingWindow {
  const parts = getLocalDateParts(timestamp, timezone);
  ...
  const end = zonedTimeToUtcIso(
    followingSaturday.getUTCFullYear(),
    followingSaturday.getUTCMonth() + 1,
    followingSaturday.getUTCDate(),
    0,
    0,
    0,
    0,
...
export function countByCategory(events: ClassifiedAgendaEvent[]): Record<AgendaEventCategory, number> {
  const counts: Record<AgendaEventCategory, number> = {
    internal_meeting: 0,
  ...
  for (const agendaEvent of events) counts[agendaEvent.category] += 1;
  return counts;
}

export function selectAgendaHighlights(events: ClassifiedAgendaEvent[], limit = DEFAULT_MAX_HIGHLIGHTS): ClassifiedAgendaEvent[] {
...
function highlightScore(event: ClassifiedAgendaEvent): number {
  let score = Number.isFinite(Date.parse(event.start)) ? 1 : 0;
  if (event.location) score += 2;
  if (event.attendeeCount && event.attendeeCount >= 3) score += 1;
  if (event.category === "customer_reception") score += 5;
  if (event.category === "outdoor_activity") score += 4;
  if (event.category === "internal_meeting") score += 2;
  if (/准备|prep|材料|review|评审/i.test(`${event.title} ${event.description ?? ""}`)) score += 2;
  return score;
}

function buildSpeech(input: {
  todayEvents: ClassifiedAgendaEvent[];
  recapEvents: ClassifiedAgendaEvent[];
  highlights: ClassifiedAgendaEvent[];
  categoryCounts: Record<AgendaEventCategory, number>;
  degraded: boolean;
}): string {
  const parts: string[] = [];
  if (input.degraded) parts.push("日历查询有部分失败，我先根据能读取到的信息给你简报。");
...
function formatHighlight(event: ClassifiedAgendaEvent): string {
...
function getLocalDateParts(timestamp: string, timezone: string): { year: number; month: number; day: number } {
...
function addUtcDays(value: number | Date, days: number): Date {
