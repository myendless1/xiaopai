...
import type {
  ProactiveCalendarRuleId,
  ProactiveCalendarTriggerType,
...
export function generateTriggerPlans(input: TriggerRuleInput): TriggerPlan[] {
  return [
...
export function createDailyBriefingPlans(context: TriggerRuleContext): TriggerPlan[] {
...
export function createMeetingStartingSoonPlans(input: TriggerRuleInput): TriggerPlan[] {
  const rule = input.rules.meetingStartingSoon;
  if (!rule.enabled) return [];
  return input.events.flatMap((event) => {
    if (!isTimedEvent(event) || !matchesMeeting(event, rule.keywords)) return [];
    return [
      buildPlan({
        context: input,
...
export function createOutdoorEventPlans(input: TriggerRuleInput): TriggerPlan[] {
  const rule = input.rules.outdoorEvent;
  if (!rule.enabled) return [];
  return input.events.flatMap((event) => {
    if (!isTimedEvent(event) || !matchesKeywords(event, rule.keywords)) return [];
    return [
      buildPlan({
        context: input,
...
export function createBusinessTripTomorrowPlans(input: TriggerRuleInput): TriggerPlan[] {
  const rule = input.rules.businessTripTomorrow;
  if (!rule.enabled) return [];
  const tomorrow = addLocalDays(getLocalDateParts(input.now, input.timezone), 1);
  const scheduledFor = zonedTimeToUtcIso(
    getLocalDateParts(input.now, input.timezone),
    parseLocalTime(rule.localTime, { hour: 18, minute: 0 }),
    input.timezone
  );
  return input.events.flatMap((event) => {
...
function normalizeCalendarEvent(event: NormalizedAgendaEvent, fallbackCalendarId: string): TriggerCalendarEventSummary {
  return {
    id: event.id,
    title: event.title,
    start: event.start,
...
function isTimedEvent(event: NormalizedAgendaEvent): boolean {
  return Number.isFinite(Date.parse(event.start)) && Number.isFinite(Date.parse(event.end));
}

function matchesMeeting(event: NormalizedAgendaEvent, keywords: string[]): boolean {
  return (event.attendeeCount ?? 0) > 1 || matchesKeywords(event, keywords) || hasLocation(event, ["会议", "meeting", "zoom", "teams"]);
}

function matchesKeywords(event: NormalizedAgendaEvent, keywords: string[]): boolean {
...
function hasLocation(event: NormalizedAgendaEvent, keywords: string[]): boolean {
