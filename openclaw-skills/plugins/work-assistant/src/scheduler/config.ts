import type { ProactiveCalendarRuleConfig, ProactiveCalendarSchedulerConfig } from "./types.js";
...
export const DEFAULT_SCHEDULER_RULES: ProactiveCalendarRuleConfig = {
  dailyBriefing: {
    enabled: true,
    localTime: "08:00"
  },
...
export function readSchedulerConfig(value: unknown): ProactiveCalendarSchedulerConfig {
...
function readRulesConfig(value: unknown): ProactiveCalendarRuleConfig {
  const raw = isRecord(value) ? value : {};
  ...
  return {
    ...
    meetingStartingSoon: {
      enabled: readBoolean(
        meetingStartingSoon.enabled,
        DEFAULT_SCHEDULER_RULES.meetingStartingSoon.enabled
      ),
      offsetMinutes:
        readPositiveNumber(meetingStartingSoon.offsetMinutes) ??
    ...
    businessTripTomorrow: {
      enabled: readBoolean(
        businessTripTomorrow.enabled,
        DEFAULT_SCHEDULER_RULES.businessTripTomorrow.enabled
      ),
      localTime:
        readLocalTime(businessTripTomorrow.localTime) ??
        DEFAULT_SCHEDULER_RULES.businessTripTomorrow.localTime,
      keywords:
        readStringArray(businessTripTomorrow.keywords) ??
...
function readLocalTime(value: unknown): string | undefined {
