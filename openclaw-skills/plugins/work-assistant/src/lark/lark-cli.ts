...
export type ProcessRunner = (argv: string[], options: { timeoutMs: number }) => Promise<ProcessResult>;
...
export function createNodeProcessRunner(cliPath = "lark-cli"): ProcessRunner {
  return (argv, options) =>
    new Promise((resolve) => {
...
function parseJson(stdout: string): unknown {
  ...
  if (trimmed === "") throw new Error("lark-cli returned empty JSON output.");
...
function normalizeUsers(payload: unknown): LarkPerson[] {
  ...
  return users.flatMap((entry) => {
    ...
    const person: LarkPerson = { id, name };
...
function extractEventId(payload: unknown): { eventId?: string; calendarId?: string; link?: string } {
  const root = asRecord(payload);
  const data = asRecord(root?.data) ?? root;
  if (!data) return {};
  const event = asRecord(data.event) ?? data;
  const result: { eventId?: string; calendarId?: string; link?: string } = {};
...
export function normalizeAgendaEvents(payload: unknown): NormalizedAgendaEvent[] {
  const root = asRecord(payload);
  const data = asRecord(root?.data);
  const arrays = [
  ...
  return entries
    ...
    .sort((left, right) => {
      ...
      if (!Number.isFinite(leftTime) && !Number.isFinite(rightTime)) return left.title.localeCompare(right.title);
...
function normalizeAgendaEvent(entry: unknown, index: number): NormalizedAgendaEvent[] {
  const record = asRecord(entry);
  if (!record) return [];
  ...
  const organizerCalendarId =
    readString(nestedEvent, "organizer_calendar_id") ?? readString(record, "organizer_calendar_id");
  const location = readLocation(nestedEvent.location) ?? readString(nestedEvent, "location");
  const description =
    readString(nestedEvent, "description") ??
    readString(nestedEvent, "remarks") ??
    readString(nestedEvent, "remark") ??
    readString(nestedEvent, "note");
  const attendeeCount =
    readNumber(nestedEvent, "attendee_count") ??
  ...
  if (normalizedCalendarId) normalized.calendarId = normalizedCalendarId;
  if (location) normalized.location = location;
  if (description) normalized.description = description;
  if (attendeeCount !== undefined) normalized.attendeeCount = attendeeCount;
  const notificationTarget = readNotificationTarget(nestedEvent);
  if (notificationTarget) normalized.notificationTarget = notificationTarget;
  return [normalized];
}

function readNotificationTarget(record: Record<string, unknown>): NormalizedAgendaEvent["notificationTarget"] | undefined {
  const target = asRecord(record.notification_target) ?? asRecord(record.notificationTarget);
  const chatId =
    (target ? readString(target, "chat_id") ?? readString(target, "chatId") : undefined) ??
  ...
  const normalized: NonNullable<NormalizedAgendaEvent["notificationTarget"]> = {};
  if (chatId) normalized.chatId = chatId;
  if (attendeeUserIds.length > 0) normalized.attendeeUserIds = attendeeUserIds;
...
function readDateLike(record: Record<string, unknown>, key: string): string | undefined {
  const value = record[key];
...
export class LarkCliContactAdapter implements LarkContactAdapter {
  ...
  async resolvePeople(names: string[]): Promise<Record<string, LarkPersonResolution>> {
    ...
    const byName: Record<string, LarkPersonResolution> = {};
...
export class LarkCliCalendarAdapter implements LarkCalendarAdapter {
  private readonly runner: ProcessRunner;
  private readonly timeoutMs: number;
  private readonly identity: "user" | "bot";
  ...
  async createEvent(request: LarkCalendarCreateRequest): Promise<LarkCalendarCreateResult> {
    ...
    if (request.description) argv.push("--description", request.description);
    if (request.attendeeIds.length > 0) argv.push("--attendee-ids", request.attendeeIds.join(","));
    ...
    try {
      ...
      const success: LarkCalendarCreateResult = {
        ok: true,
        eventId: event.eventId ?? "unknown"
      };
      if (event.calendarId) success.calendarId = event.calendarId;
      if (event.link) success.link = event.link;
      return success;
    } catch (error) {
      return {
        ok: false,
...
export class LarkCliIMAdapter implements LarkIMAdapter {
  private readonly runner: ProcessRunner;
  private readonly timeoutMs: number;
  private readonly identity: "user" | "bot";
  ...
  async sendText(request: LarkMessageSendRequest): Promise<LarkMessageSendResult> {
    ...
    if (request.idempotencyKey) argv.push("--idempotency-key", request.idempotencyKey);
...
function buildMessageTargetArgv(request: LarkMessageSendRequest): string[] | undefined {
  if (request.chatId) return ["--chat-id", request.chatId];
  const firstUserId = request.attendeeUserIds?.[0];
  if (firstUserId) return ["--user-id", firstUserId];
  return undefined;
}
