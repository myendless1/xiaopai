import type { InputEvent, StructuredCalendarCreateIntent, StructuredResponse, ToolAction } from "../contracts.js";
import { followUpResponse, validateStructuredAssistantIntent } from "../contracts.js";
import type { LarkCalendarAdapter, LarkContactAdapter, LarkPersonResolution } from "../lark/adapters.js";
import { compareIso } from "./time.js";
import { CalendarIntentParser, type CalendarCreateIntent } from "./parser.js";
...
type NormalizedCalendarCreateIntent = {
  title?: string;
...
export class CalendarAssistant {
  private readonly contactAdapter: LarkContactAdapter;
  private readonly calendarAdapter: LarkCalendarAdapter;
  private readonly parser: CalendarIntentParser;
  ...
  async handle(event: InputEvent): Promise<StructuredResponse> {
    ...
    if (attendeeIssue) return attendeeIssue;

    const resolvedAttendeeIds = intent.attendeeNames.map((name) => {
      const resolution = resolutions[name];
      if (!resolution || resolution.status !== "unique") throw new Error(`Unexpected unresolved attendee: ${name}`);
...
function normalizeCalendarInput(
  event: InputEvent,
  parser: CalendarIntentParser
): { intent: NormalizedCalendarCreateIntent } | { response: StructuredResponse } {
  if (Object.prototype.hasOwnProperty.call(event.payload, "structured_intent")) {
    ...
    if (!validation.ok) {
      return {
        response: followUpResponse(
          structuredIntentQuestion(validation.reason),
          validation.reason,
          "结构化日程请求不完整或无效，请补充后再试。"
        )
      };
    }
    if (validation.value.type !== "calendar.create") {
      return {
        response: followUpResponse(
          "这个结构化请求不是日程创建请求，请改为 calendar.create 日程创建请求。",
...
function fromStructuredIntent(intent: StructuredCalendarCreateIntent): NormalizedCalendarCreateIntent {
  return {
    title: intent.title,
    start: intent.start,
    end: intent.end,
...
function fromParserIntent(intent: CalendarCreateIntent): NormalizedCalendarCreateIntent {
  const normalized: NormalizedCalendarCreateIntent = {
    attendeeNames: intent.attendeeNames,
    attendeeIds: [],
    confidence: intent.confidence,
    source: "parser"
  };
  if (intent.title) normalized.title = intent.title;
  if (intent.start) normalized.start = intent.start;
  if (intent.end) normalized.end = intent.end;
  return normalized;
}

function structuredIntentQuestion(reason: string): string {
  switch (reason) {
...
function validateIntent(intent: NormalizedCalendarCreateIntent, event: InputEvent): StructuredResponse | undefined {
  if (!intent.title) {
...
function validateAttendeeResolutions(resolutions: Record<string, LarkPersonResolution>): StructuredResponse | undefined {
