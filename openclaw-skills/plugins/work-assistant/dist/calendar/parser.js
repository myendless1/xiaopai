import { localIso, resolveRelativeDate } from "./time.js";
export class CalendarIntentParser {
    parse(event) {
        const text = typeof event.payload.text === "string" ? event.payload.text.trim() : "";
        if (!text)
            return { attendeeNames: [], confidence: 0, ambiguity: "missing_text" };
        ...
        if (title)
            intent.title = title;
        if (date && times?.start)
            intent.start = localIso(date, times.start, event.context.timezone);
        if (date && times?.end)
            intent.end = localIso(date, times.end, event.context.timezone);
        if (!date || !times)
            intent.ambiguity = "missing_time";
        return intent;
    }
    extractTitle(text) {
        const titleMatch = /(?:今天|明天|后天).{0,16}?点(?:半)?(?:到|-|至).{1,12}?点(?:半)?的(.+?)(?:，|,|。|帮我|请|$)/.exec(text) ??
