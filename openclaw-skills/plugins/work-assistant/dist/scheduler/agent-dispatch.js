...
function selectSchedulerPromptProfile(eventType) {
    switch (eventType) {
        case "daily_briefing_triggered":
            return {
                label: "定时日程管理及播报",
                speechStyle: "以轻快早安风格播报，例如“早上好呀！今天是x月x日（周x），新的一天开始啦！上周工作为您复盘....。建议本周....”。如果输入中没有上周复盘数据，不要编造上周事实，可自然改成“我先根据今天的日程帮您快速过一遍”。",
        ...
        case "meeting_starting_soon":
            return {
                label: "会议会前提醒",
                speechStyle: "以会前轻提醒风格播报，例如“打扰一下，xxx点 xx 会议还有 x 分钟将在 xxx 会议室开始，请提前准备参会”。如果没有会议室信息，就说“会议地点待确认”或按输入中的实际地点表达。",
                presentationInstructions: [
        ...
        case "outdoor_event_detected":
            return {
                label: "日常外勤出行规划",
                speechStyle: "以亲切外勤提醒风格播报，例如“哈喽，您下午 xxx 点有外出行程，目的地是xxxxx，请提前预留时间哦”。如果实际不是下午，请按输入中的真实时间段表达。",
                presentationInstructions: [
                    "微笑表情使用 xiaopai-control 支持的 face expression: happy_squint。",
                    "sequence 开始先调用 { \"type\": \"face\", \"expression\": \"happy_squint\" }。"
                ]
        ...
        case "business_trip_tomorrow_detected":
            return {
                label: "异地出差出行建议",
                speechStyle: "以下班前轻松提醒风格播报，例如“哈喽，准备下班咯~提醒您明早 x 点将前往 xx 出差，xx 明日天气...，与深圳温差较大哦~”。如果输入中没有深圳天气或温差数据，不要断言温差较大，可说“也留意和深圳的体感差异哦”。",
                presentationInstructions: [
                    "微笑表情使用 xiaopai-control 支持的 face expression: happy_squint。",
                    "sequence 开始先调用 { \"type\": \"face\", \"expression\": \"happy_squint\" }。"
                ]
...
function collectEventFacts(event, response) {
    const facts = [];
    ...
    if (minutesUntilStart !== undefined)
        facts.push(`距离日程开始: ${minutesUntilStart} 分钟`);
    ...
    if (facts.length === 0)
        facts.push("无额外结构化事实，请以 Work Assistant 原始播报为准");
...
function calculateMinutesUntilStart(firedAt, startsAt) {
    if (typeof firedAt !== "string" || typeof startsAt !== "string")
        return undefined;
    const fired = Date.parse(firedAt);
    const start = Date.parse(startsAt);
    if (!Number.isFinite(fired) || !Number.isFinite(start))
        return undefined;
