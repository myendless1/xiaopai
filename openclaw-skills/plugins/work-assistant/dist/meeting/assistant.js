...
export class MeetingReminderAssistant {
    ...
    async handleLateNotification(event) {
...
export function shouldRouteToMeetingNotification(event) {
    if (event.type !== "user_utterance")
        return false;
    const structuredIntent = event.payload.structured_intent;
    if (isRecord(structuredIntent) && structuredIntent.type === "meeting.notify_late")
        return true;
...
function normalizeLateNotificationRequest(event) {
    if (Object.prototype.hasOwnProperty.call(event.payload, "structured_intent")) {
        ...
        if (!validation.ok) {
            return {
                response: followUpResponse(validation.reason === "unsupported_intent_version"
                    ? "这个迟到通知请求版本暂不支持，请使用 version 为 1 的格式。"
                    : "迟到通知请求格式无效，请补充后再试。", validation.reason, "结构化迟到通知请求不完整或无效，请补充后再试。")
            };
        }
...
function fromStructuredIntent(intent) {
    const result = { source: "structured" };
    if (intent.delay_minutes !== undefined)
        result.delayMinutes = intent.delay_minutes;
    if (intent.message)
        result.message = intent.message;
    return result;
}
function parseLateNotificationFallback(text) {
    if (!looksLikeLateNotification(text))
        return undefined;
...
function looksLikeLateNotification(text) {
...
function extractDelayMinutes(text) {
    ...
    if (digitMatch?.[1])
        return clampDelay(Number(digitMatch[1]));
...
function formatLateNotificationMessage(intent, focus) {
    if (intent.message)
        return intent.message.slice(0, 200);
    const delay = intent.delayMinutes ? `我会晚 ${intent.delayMinutes} 分钟到` : "我会晚一点到";
    return `${delay}，请大家先开始或稍等一下。`;
}
function buildReminderSpeech(event, timezone) {
...
function buildCurrentFocus(event) {
    ...
    if (event.calendar_id)
        focus.calendar_id = event.calendar_id;
    if (event.location)
        focus.location = event.location;
...
function normalizeCurrentMeetingFocus(value) {
    ...
    if (!record || record.type !== "calendar_event")
        return undefined;
...
function normalizeCalendarEvent(value) {
    ...
    if (attendeeUserIds.length > 0)
        event.attendee_user_ids = attendeeUserIds;
    if (target)
        event.notification_target = target;
    return event;
}
function normalizeNotificationTarget(value) {
    ...
    if (attendeeUserIds.length > 0)
        target.attendee_user_ids = attendeeUserIds;
