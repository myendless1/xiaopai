...
export class TravelPlannerAssistant {
    ...
    async handleOutdoorEvent(event) {
        ...
        if (!preferences.originAddress) {
            ...
            if (routeAction.route) {
                ...
                recommendedDepartureTime = calculateDepartureTime(calendarEvent.start, route.durationMinutes, preferences.arrivalBufferMinutes);
            }
        }
        actions.push({
    ...
    async handleBusinessTripTomorrow(event) {
        const calendarEvent = normalizeCalendarEvent(event.payload.calendar_event, event.payload.trigger);
        if (!calendarEvent)
            return malformedResponse("business_trip");
        ...
        return {
            speech: buildBusinessTripSpeech(calendarEvent, destination.destination, event.context.timezone, forecastAction.forecast),
            presentation: {
    ...
    async readPreferences(userId) {
        try {
            ...
            if (merged.originAddress)
                response.originAddress = merged.originAddress;
            return response;
        }
        catch (error) {
            return {
    ...
    async estimateRoute(calendarEvent, destination, preferences) {
        if (!preferences.originAddress) {
            return {
    ...
    async getForecast(calendarEvent, destination) {
        try {
...
function normalizeCalendarEvent(value, triggerValue) {
    if (!isRecord(value))
        return undefined;
    ...
    if (!id || !title || !start || !end || !isValidDate(start) || !isValidDate(end))
        return undefined;
    const event = {
        id,
        title,
    ...
    if (trigger?.triggerKey)
        event.triggerKey = trigger.triggerKey;
    if (trigger?.sourceEventId)
        event.sourceEventId = trigger.sourceEventId;
    return event;
}
function normalizeTrigger(value) {
    ...
    if (triggerKey)
        result.triggerKey = triggerKey;
    if (sourceEventId)
        result.sourceEventId = sourceEventId;
    return result;
}
function resolveDestination(event, kind) {
    if (event.location)
        return { destination: event.location, source: "location" };
    const fromDescription = extractDestinationFromText(event.description);
    if (fromDescription)
        return { destination: fromDescription, source: "description" };
...
function extractDestinationFromText(value) {
    if (!value)
        return undefined;
    const match = value.match(/(?:前往|去|到|赴|地点[:：]?)([^，。；;,\n]{2,30})/);
    return cleanDestination(match?.[1]);
}
function extractOutdoorDestinationFromTitle(title) {
...
function extractTripDestinationFromTitle(title) {
    const explicit = title.match(/(?:前往|去|到|赴)([^，。；;,\n]{2,30})/);
    const explicitDestination = cleanDestination(explicit?.[1]);
    if (explicitDestination)
        return explicitDestination;
...
function malformedResponse(kind) {
...
function outdoorMissingDestinationResponse(event) {
    return {
...
function businessTripMissingDestinationResponse(event, timezone) {
    return {
...
function buildOutdoorSpeech(event, destination, timezone, route, recommendedDepartureTime) {
    const time = formatLocalTime(event.start, timezone);
    const descriptionNote = extractPreparationNote(event.description);
    const routeText = route && recommendedDepartureTime
...
function buildBusinessTripSpeech(event, destination, timezone, forecast) {
    const time = formatLocalTime(event.start, timezone);
...
function buildPreparationNotes(event, forecast) {
...
function weatherPreparationNote(forecast) {
    if (!forecast)
        return undefined;
    if (forecast.condition === "rain" || (forecast.precipitationChance ?? 0) >= 50)
        return "雨具";
    if (forecast.condition === "cold" || (forecast.lowCelsius ?? 99) <= 8)
        return "外套";
    if (forecast.condition === "hot" || (forecast.highCelsius ?? 0) >= 32)
        return "轻便衣物";
    if (forecast.lowCelsius !== undefined &&
...
function extractPreparationNote(description) {
    if (!description)
        return undefined;
...
function skippedRouteAction(event, destination, code, message) {
    return {
        type: "route.estimate",
        status: "skipped",
        error: {
...
function routeActionDetails(event, destination, preferences) {
    return {
        event_id: event.id,
        title: event.title,
        origin: preferences.originAddress,
...
function weatherActionDetails(event, destination) {
    return {
        event_id: event.id,
        title: event.title,
        destination: destination.destination,
...
function buildTravelFocus(event, destination, recommendedDepartureTime) {
    const focus = {
        type: "travel_event",
        event_id: event.id,
        title: event.title,
    ...
    if (event.calendarId)
        focus.calendar_id = event.calendarId;
    if (recommendedDepartureTime)
        focus.recommended_departure_time = recommendedDepartureTime;
    return focus;
}
function calculateDepartureTime(start, durationMinutes, bufferMinutes) {
...
function mergeProfile(config, profile) {
    const merged = {};
    if (profile.originAddress)
        merged.originAddress = profile.originAddress;
    if (config.originAddress)
        merged.originAddress = config.originAddress;
    if (profile.homeCity)
        merged.homeCity = profile.homeCity;
    if (profile.defaultRouteMode)
        merged.defaultRouteMode = profile.defaultRouteMode;
    if (config.defaultRouteMode)
        merged.defaultRouteMode = config.defaultRouteMode;
    if (profile.arrivalBufferMinutes)
        merged.arrivalBufferMinutes = profile.arrivalBufferMinutes;
    if (config.arrivalBufferMinutes)
        merged.arrivalBufferMinutes = config.arrivalBufferMinutes;
    return merged;
}
