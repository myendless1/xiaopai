...
export function normalizeSpeechTextForVoice(text) {
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        if (isMarkdownTableRow(tableLine) && isMarkdownTableSeparatorRow(nextLine)) {
            ...
            while (index < lines.length) {
                ...
                if (!isMarkdownTableRow(rowLine) || isMarkdownTableSeparatorRow(rowLine))
                    break;
            ...
            index -= 1;
            if (rows.length > 0)
                output.push(withSentenceEnding(rows.join("；")));
            continue;
        }
...
function splitTablePrefix(line, nextLine) {
...
function formatMarkdownTableRow(headers, cells) {
    const parts = [];
...
function isHeaderSafeToOmit(header) {
...
function withSentenceEnding(value) {
    ...
    if (trimmed === "" || SPEECH_ENDING_PUNCTUATION.test(trimmed))
        return trimmed;
    return `${trimmed}。`;
}
