...
const directions = new Set<string>(XIAOPAI_MOVE_DIRECTIONS);

export function extractCommandInput(value: unknown): unknown {
  if (isRecord(value) && "command" in value) return value.command;
  return value;
}

export function validateXiaopaiCommand(value: unknown): ValidationResult<XiaopaiCommand> {
  const command = validateCommandRecord(value, undefined);
  if (!command.ok) return command;
  return { ok: true, value: command.value as XiaopaiCommand };
}

function validateCommandRecord(value: unknown, stepIndex: number | undefined): ValidationResult<XiaopaiCommand | XiaopaiSequenceStep> {
  if (!isRecord(value)) return invalid("invalid_command", "Command must be an object.", "command", stepIndex);
  ...
  switch (value.type) {
    ...
    case "sequence":
      if (stepIndex !== undefined) return invalid("nested_sequence_unsupported", "Sequence steps cannot be nested sequences.", "type", stepIndex, value.type);
      return validateSequence(value);
    case "stop":
      if (stepIndex !== undefined) return invalid("sequence_stop_unsupported", "Sequence steps cannot include stop.", "type", stepIndex, value.type);
...
function validateSpeak(value: Record<string, unknown>, stepIndex: number | undefined): ValidationResult<XiaopaiSequenceStep> {
...
function validateFace(value: Record<string, unknown>, stepIndex: number | undefined): ValidationResult<XiaopaiSequenceStep> {
  ...
  const base = validateBase(value, stepIndex);
  if (!base.ok) return base;
  return { ok: true, value: { type: "face", expression: value.expression as XiaopaiExpression, ...base.value } };
}

function validateAction(value: Record<string, unknown>, stepIndex: number | undefined): ValidationResult<XiaopaiSequenceStep> {
  ...
  const base = validateBase(value, stepIndex);
  if (!base.ok) return base;
  return { ok: true, value: { type: "action", action: value.action as XiaopaiAction, ...base.value } };
}

function validateMove(value: Record<string, unknown>, stepIndex: number | undefined): ValidationResult<XiaopaiSequenceStep> {
  ...
  if (!degree.ok) return invalid("invalid_move_degree", `move.degree must be between ${MIN_MOVE_DEGREE} and ${MAX_MOVE_DEGREE}.`, "degree", stepIndex, value.degree);
  ...
  if (!duration.ok) {
    return invalid(
      ...
      `move.duration_ms must be between ${MIN_MOVE_DURATION_MS} and ${MAX_MOVE_DURATION_MS}.`,
      "duration_ms",
      stepIndex,
      value.duration_ms
    );
  ...
  return {
    ...
    value: {
      type: "move",
      direction: direction as XiaopaiMoveDirection,
...
function validateSequence(value: Record<string, unknown>): ValidationResult<XiaopaiCommand> {
  ...
  if (!base.ok) return base;
  const steps: XiaopaiSequenceStep[] = [];
...
function validateStop(value: Record<string, unknown>): ValidationResult<XiaopaiCommand> {
  const base = validateBase(value, undefined);
  if (!base.ok) return base;
...
function validateBase(value: Record<string, unknown>, stepIndex: number | undefined): ValidationResult<{ device_id?: string; interrupt?: boolean }> {
  if (stepIndex !== undefined) return { ok: true, value: {} };
  const result: { device_id?: string; interrupt?: boolean } = {};
...
function stripTopLevelFields(command: XiaopaiCommand | XiaopaiSequenceStep): XiaopaiSequenceStep {
  const { device_id: _deviceId, interrupt: _interrupt, ...step } = command as XiaopaiCommand & { device_id?: string; interrupt?: boolean };
  return step as XiaopaiSequenceStep;
}

function readOptionalBoundedNumber(value: unknown, min: number, max: number): { ok: true; value: number | undefined } | { ok: false } {
  if (value === undefined) return { ok: true, value: undefined };
  if (typeof value !== "number" || !Number.isFinite(value) || value < min || value > max) return { ok: false };
  return { ok: true, value };
}

function invalid(code: string, message: string, field?: string, stepIndex?: number, value?: unknown): ValidationResult<never> {
  const error: XiaopaiValidationError = {
    code,
    message,
    ...(field ? { field } : {}),
...
export function rejectedResultForValidation(error: XiaopaiValidationError) {
