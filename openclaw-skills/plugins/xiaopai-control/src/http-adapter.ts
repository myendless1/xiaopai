...
import type {
  XiaopaiCommand,
  XiaopaiCommandResult,
  XiaopaiDevice,
  XiaopaiDeviceListResult,
  XiaopaiHealthResult,
  XiaopaiSequenceStep
} from "./contracts.js";
import { failedCommandResult, queuedCommandResult } from "./results.js";

export class XiaopaiHttpAdapter implements XiaopaiControlAdapter {
  ...
  private async postCommand(command: Exclude<XiaopaiCommand, { type: "stop" }>): Promise<XiaopaiCommandResult> {
  ...
  private async stop(command: XiaopaiCommand & { type: "stop" }): Promise<XiaopaiCommandResult> {
  ...
  private async requestJson(method: "GET" | "POST", path: string, body?: unknown): Promise<JsonRequestResult> {
...
function toStackChanPostBody(command: Exclude<XiaopaiCommand, { type: "stop" }>): Record<string, unknown> {
  const base = {
    type: command.type,
...
function toStackChanSequencePayload(steps: XiaopaiSequenceStep[]): Record<string, unknown>[] {
  ...
  for (let index = 0; index < wireSteps.length; index += 1) {
    ...
    if (step?.type === "action" && nextStep?.type === "face" && nextWireStep && !hasSpeakAfter(steps, index + 1)) {
      ...
      if (nextStep.expression !== "calm") result.push(nextWireStep);
...
function toStackChanSequenceStep(step: XiaopaiSequenceStep): Record<string, unknown> {
  switch (step.type) {
    case "speak":
...
function normalizeQueueResponse(command: XiaopaiCommand, body: unknown): XiaopaiCommandResult {
...
function normalizeDevice(value: unknown): XiaopaiDevice[] {
  if (!isRecord(value) || typeof value.device_id !== "string") return [];
  return [
    {
...
function normalizeFetchError(error: unknown): JsonRequestResult {
