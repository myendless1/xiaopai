...
Expressions: `calm`, `sleep_dark`, `screen_off`, `shy`, `thinking`, `relaxed`, `smile_blink`, `blink_half`, `blink_closed`, `wink_half`, `wink_closed`, `heart_small`, `heart`, `nod_soft`, `nod_down`, `happy_squint`, `happy_squint_soft`.

Actions: `blink`, `wink`, `heart_action`, `hearting`, `nod`, `nodding`, `happy_dynamic`, `happy_squint_dynamic`, `node_head`, `nod_head`.
...
When a Work Assistant scheduler prompt is delivered as plain natural language and says it is a `Work Assistant 调度器生成的被动播报任务`, there is no stack-chan JSON render fallback. Follow the prompt directly: use the supplied task facts and requested speech style, then explicitly call `xiaopaiControl.execute` with one `sequence` command. The prompt may specify exact supported values such as `face: happy_squint` for a smile, or `face: smile_blink` plus `action: blink` for a blinking dynamic expression. Use those exact values when present, include the target `device_id` and `interrupt` flag if provided, include a non-empty `speak` step for the final spoken text, and finish with `face: calm`.
